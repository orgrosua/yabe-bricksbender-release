...
namespace _YabeBricksbender\Kucrut\Vite;
...
function load_development_asset(object $manifest, string $entry, array $options) : ?array
{
    ...
    $dependencies = \array_merge([VITE_CLIENT_SCRIPT_HANDLE], $options['dependencies']);
...
function register_asset(string $manifest_dir, string $entry, array $options) : ?array
{
    ...
    $assets = $manifest->is_dev ? load_development_asset($manifest, $entry, $options) : load_production_asset($manifest, $entry, $options);
    return $assets;
}
/**
 * Enqueue asset
