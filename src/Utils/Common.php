...
class Common
{
    ...
    public static function plugin_data(?string $key = null)
    {
        ...
        $plugin_data = \wp_cache_get('plugin_data', BRICKSBENDER::WP_OPTION);
        if (!$plugin_data) {
            $plugin_data = \get_plugin_data(BRICKSBENDER::FILE);
            \wp_cache_set('plugin_data', $plugin_data, BRICKSBENDER::WP_OPTION);
        }
        return $key ? $plugin_data[$key] : $plugin_data;
    }
    public static function random_slug(int $length = 21) : string
    {
        return (new \_YabeBricksbender\Hidehalo\Nanoid\Client())->generateId($length, \_YabeBricksbender\Hidehalo\Nanoid\Client::MODE_DYNAMIC);
    }
    /**
