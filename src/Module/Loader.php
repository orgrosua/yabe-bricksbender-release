...
use Exception;
use ReflectionClass;
use _YabeBricksbender\Symfony\Component\Finder\Finder;
class Loader
{
    /**
    ...
    public function scan_modules()
    {
        ...
        foreach ($declared_classes as $declared_class) {
            ...
            if (!$reflector->isSubclassOf(\Yabe\Bricksbender\Module\ModuleInterface::class)) {
                continue;
            }
            ...
            $this->modules[$module->get_name()] = ['name' => $module->get_name(), 'file_path' => $reflector->getFileName(), 'class_name' => $reflector->getName()];
        }
        return $this;
    }
    /**
