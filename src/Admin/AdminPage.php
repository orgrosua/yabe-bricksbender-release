...
class AdminPage
{
    ...
    {
        return \add_query_arg(['page' => BRICKSBENDER::WP_OPTION], \admin_url('admin.php'));
    }
    public function add_admin_menu()
    {
        $hook = \add_submenu_page('bricks', \__('Yabe Bricksbender', 'yabe-bricksbender'), \__('Yabe Bricksbender', 'yabe-bricksbender'), 'manage_options', BRICKSBENDER::WP_OPTION, fn() => $this->render(), 1000001);
        \add_action('load-' . $hook, fn() => $this->init_hooks());
    }
    private function render()
    {
        \add_filter('admin_footer_text', static fn($text) => 'Thank you for using <b>Bricksbender</b>! Join us on the <a href="https://www.facebook.com/groups/1142662969627943" target="_blank">Facebook Group</a>.', 1000001);
        \add_filter('update_footer', static fn($text) => $text . ' | Bricksbender ' . BRICKSBENDER::VERSION, 1000001);
